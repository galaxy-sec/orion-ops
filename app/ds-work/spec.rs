...
use orion_syspec::workins::proj::WorkInsProject;

use crate::args::GInsCmd;

pub async fn do_sys_cmd(cmd: GInsCmd) -> SpecResult<()> {
    let current_dir = std::env::current_dir().expect("无法获取当前目录");
    match cmd {
        GInsCmd::New(args) => {
            let new_prj = current_dir.join(args.name());
            make_new_path(&new_prj)?;
            let spec = WorkInsProject::make_new(&new_prj, args.name()).err_conv()?;
            spec.save().err_conv()?;
        }
        GInsCmd::Update(dfx) => {
            configure_dfx_logging(&dfx);
            let options = UpdateOptions::from((dfx.force, ValueDict::default()));
            let spec = WorkInsProject::load(&current_dir).err_conv()?;

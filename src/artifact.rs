...
use orion_variate::addr::{AddrResult, AddrType};
use orion_variate::types::{ResourceUpload, UnitUpdateValue, UnitUpdateable};
    ...
    pub async fn release_repo_to_transit(
        ...
        if let Some(AddrType::Local(local)) = self.transit_storage() {
            ...
            let result = if let Some(release) = self.release_repo() {
                ...
                UnitUpdateValue::from(local_path.to_path_buf())
            };
            Ok(result)
        } else {
...
mod tests {
    ...
    use orion_variate::addr::{GitAddr, HttpAddr, LocalAddr};
